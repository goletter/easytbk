...
namespace Goletter\EasyTBK\vip\Osp\Base;

//use Thrift\Transport\TFramedTransport;
//use Thrift\Type\TMessageType;
//use Thrift\Transport\TSocket;
//use Thrift\Transport\TBufferedTransport;
use Goletter\EasyTBK\vip\Osp\Context\InvocationContextFactory;
...
class OspStub
{
    ...
    {
        \Goletter\EasyTBK\vip\Osp\Log\Logger::info("[OspStub][InitInvocation][Service-Version-Method][" . $this->serviceName . "-" . $this->version . "-" . $method . "]");
        ...
        , $timestamp, $version, $request, $appSecret)
    {
        ...
        return \Goletter\EasyTBK\vip\Osp\Util\HmacUtil::hmac($sign, $appSecret);
    }
    ...
    {
        if (!$restAddr) throw new \Goletter\EasyTBK\vip\Osp\Exception\OspException("The openapi appURL is null, please set the appURL value");
    ...
    {
        $ctx = InvocationContextFactory::getInstance();
        $oprot = new \Goletter\EasyTBK\vip\Osp\Protocol\JSONProtocol(new \Goletter\EasyTBK\vip\Osp\Buffer\MemoryBuffer());
        $args->write($oprot);
        ...
        $client = new \Goletter\EasyTBK\vip\Osp\Http\HttpClient($scheme, $host, $port);
        $request = array();

        $appKey = $ctx->getAppKey();
        if ($appKey == null) throw new \Goletter\EasyTBK\vip\Osp\Exception\OspException("The openapi appKey is null, please set the appKey value");

        $appSecret = $ctx->getAppSecret();
        if ($appSecret == null) throw new \Goletter\EasyTBK\vip\Osp\Exception\OspException("The openapi appSecret is null, please set the appSecret value");
        ...
        $timestamp = round(\Goletter\EasyTBK\vip\Osp\Util\TimeUtil::currentTimeMillis() / 1000);
        ...
        try {
            ...
            throw new \Goletter\EasyTBK\vip\Osp\Exception\OspException(\Osp\Util\MessageUtil::getInvocationMsg($ctx, $e->getMessage()), $e);
        }
    ...
    public function receive_base($result)
    {
        ...
        if (\Goletter\EasyTBK\vip\Osp\Base\OspStub::$RETURN_NULL != $response) {
            $iprot = new \Goletter\EasyTBK\vip\Osp\Protocol\JSONProtocol(new \Goletter\EasyTBK\vip\Osp\Buffer\MemoryBuffer());
            ...
            if (!"0" == $returnCode) {
                ...
                throw new \Goletter\EasyTBK\vip\Osp\Exception\OspException($returnMessage, $returnCode);
            }
